...
use crate::events::{Event, MonthDay};
use crate::providers::{
    EventProvider, 
...
use crate::filters::FilterBuilder;  // 0.32.0
...
pub fn run(config: &Config, config_path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    // Make a new filter with today's date as the month-day:
    ...
    // Create the event providers and get events from them, filtered:
    ...
    // Print out the filtered list of events (still in no particular order):
    for event in events {
        println!("{}: {} ({})",
            event.year(),
            event.description(),
